...
export const FloatingNav = ({
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        className={cn(
          "mx-auto mt-24 flex w-max items-center justify-center gap-5 rounded-xl border border-white/20 p-2 backdrop-blur-xl lg:mt-5 lg:flex-col",
        ...
        {navItems.map(({ link, name, icon }, idx: number) => (
          <TooltipProvider key={`link=${idx}`} delayDuration={0}>
            <Tooltip>
              <TooltipTrigger
                className={cn(
                  `flex items-center rounded-md text-2xl shadow-none hover:bg-white/40`,
                  `${pathname === link && link !== "" ? "bg-white/40" : ""}`,
                )}
                asChild
              >
                {link !== "" ? (
